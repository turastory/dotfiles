...
- **각 코멘트 첫 줄에 메타 태그 라인을 붙인다.** `🏷 <카테고리> · <경중> · <연관도>` 한 줄을 본문 맨 위에 두고 빈 줄로 띄운다 (값·기준은 아래 `코멘트 메타 태그` 참조).
...
- 코멘트를 다 추가한 뒤에는 어떤 파일·라인에 무슨 코멘트를 남겼는지 요약하되, **연관도 순(`이번 PR` → `이번 PR 권장` → `다음에 해도 됨`)으로 정렬**해 사용자가 submit 전에 "지금 꼭 할 것"과 "넘겨도 될 것"을 한눈에 구분하게 한다.

## 코멘트 메타 태그

각 코멘트 첫 줄에 `🏷 <카테고리> · <경중> · <연관도>` 한 줄을 붙여, 받는 사람이 본문을 다 읽기 전에 성격·우선순위를 알 수 있게 한다. 본문은 `my-tone` 톤 그대로 둔다. 태그가 본문과 어긋나지 않게(예: `사소`인데 본문이 "꼭 고쳐야"로 읽히면 안 됨) 맞춘다.

- **카테고리** (성격, 카테고리별 판단 기준과 매핑): `로직` / `가독성` / `네이밍` / `컨벤션` / `테스트` / `타입` / `성능` / `범위` / `문서`. 두 개에 걸치면 핵심 하나만 고른다.
- **경중** (문제의 심각도, 봇의 Severity와 겹치면 봇 판정 참고): `심각`(데이터 유실·결제 오류 등 사용자 영향 큰 버그) / `중요`(동작·정합성 문제지만 영향 한정) / `사소`(개선·취향·유지보수).
- **연관도** (이번 PR에서 처리해야 하는지):
  - `이번 PR` — 이 PR이 만든 문제거나 머지 전 막아야 함.
  - `이번 PR 권장` — 직접 관련 있고 가능하면 이번에 같이. 안 되면 미뤄도 됨.
  - `다음에 해도 됨 👀` — 직접 관련은 아님. optional·후속 PR로 넘겨도 무방. 본문 종결도 "넘겨도 됩니다", "optional로 봐주세요"로 맞춘다.
- 경중과 연관도는 독립이다(사소한데 이번 PR에서 막아야 할 수도, 중요한데 후속으로 빼도 될 수도 있음). 따로 판단한다.
...
여기서는 `isPointback`만 보고 토글을 숨기는데, 위 배너는 `orderType === 'rental'`까지 봅니다. 대여 pointback이 캐시된 채 소장 탭으로 전환되면 소장 결제에서도 토글이 숨을 수 있어서, 토글 조건도 `orderType === 'rental'`로 좁히는 게 안전해 보입니다.
...
2. **pending review 갱신**: 새 finding을 `event` 없이 pending으로 추가한다(`코멘트 게시 방식`). 각 코멘트 첫 줄에 메타 태그(`코멘트 메타 태그`)를 붙인다.
